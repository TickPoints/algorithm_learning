...
\end{align*}
$$
该式可以避免溢出(计算`low + high`可能造成数值过大, `high - low`会更好). [^note1]
...
二分查找显然使用了分治法[^note2], 不过比较特殊的是它没有合并的步骤(并且采用迭代法). 我们利用[循环不变式](./loop_invariant.md)来证明二分查找的正确性:
...
[^note1]: 事实上, [标准库的实现](https://www.rustwiki.org.cn/zh-CN/src/core/slice/mod.rs.html#2779-2785)也是这么做的.

[^note2]: 二分查找是通过缩小搜索区间来工作, 而不是逐步构建解, 因此不属于增量方法. 它属于分治法的一种特例, 我们称作 **减治法(decrease and conquer)**.

[^note3]: **Timsort** 是一种高效的混合排序算法, 由 **提姆·彼得斯(Tim Peters)** 在 2002 年为 Python 语言设计(Python 2.3 及后续版本的默认排序算法). 其针对现实世界中的部分有序数据进行了优化, 具有稳定性和适应性.

[^note4]: 即$k$的增长速度不超过$\log n$, 这里读者可以先行自证, 此类标准的证明方法在后续讲到.

...
在Rust我们做不到使用$\infty$(除浮点数), 哪怕用[`Default::default()`](https://rustwiki.org/zh-CN/std/default/trait.Default.html#tymethod.default)也会导致数据冲突(如数字`0`), 所以我们不采用**哨兵值**. 如果你认为你的数据里不会产生`0`或`-1`这样的值, 那加上哨兵值也是可取的.

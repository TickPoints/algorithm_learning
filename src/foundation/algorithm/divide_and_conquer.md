...
对于这个问题，我们进行分割将数组分割成两个`n/2`的子数组，然后对这两个子数组重复上面分割。不可分割时，需进行排序，然后合并已排序的数组。这是一个先**自上而下**分解问题，再逐步回升解决问题的过程。
### 实现1
```rs
...
new: 1 2 3 4 7 8
```
当然，这只是一种思维方式。实际的实现中，我们通常不改变左右数组，而是简单的通过一个索引来从中获取数据并模拟扔出数据的过程。这个过程需要遍历两个数组的所有元素，所以它是\\(\Theta(n)\\)[^note3]的。
...
[^note3]: 这只是辅助函数的时间复杂度，归并排序的实际复杂度在后面解答。

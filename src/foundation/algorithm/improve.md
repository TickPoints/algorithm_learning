...
从上面的循环不变式不难看出: 冒泡排序通过两两互相比较, 每次都将这次最大的数据放到后面. 很显然, 它用的是跟插入排序一样的增量法, 时间复杂度也为$T(n) = n ^2$.
...
该算法用了`fold()`一次, 不难看出时间复杂度为$\Theta(n)$[^note1], 我们写个普通运算:
```rs
...
对于寻找"哪些逆序对", 该时间复杂度不能再降了. 因为最坏情况下有$\Theta(n ^ 2)$[^note2]的逆序对, 输出它们并需要$\Theta(n ^ 2)$的时间复杂度. 那试考虑 **寻找"有几个逆序对"有无更低时间复杂度的解**:
...
[^note1]: 习惯上, 对于一些时间复杂度相对较低的外部函数(比如IO操作的函数)我们通常将其忽略; 另一方面, 对于迭代器里面的多数函数, 它们通常具有较高的时间复杂度, 我们要正常考虑.

[^note2]: 这里指 **空间复杂度**, 后面的则仍为时间复杂度. 空间复杂度会在后面介绍.

...
这个循环不变式比较简单，留给读者自证。显然，我们不难证得$T(n) = \Theta(\log n)$。
...
```
这里主要逻辑改写在比较部分: 当`arr[mid] < v`时，目标值必然在右侧，所以移动`low`；当`arr[mid] >= v`目标值可能出现在左侧或当前位置(`arr[mid] == v`)，所以移动`high`到`mid`(不是`mid - 1`)。重点在`low >= high`说明所有的`v`都已出现(这里就是上面`low < high`不使用等号的原因)，那么`low`必然在最小`v`的位置上。当然这个算法有个问题，不存在时会误报，所以要二次判断。同理不难写出寻找最大`i`的二分算法:
### 实现4
```rust
...
```
可以发现，比较逻辑的改写比较复杂，所以循环不变式在判断二分算法的正确性上非常重要。
